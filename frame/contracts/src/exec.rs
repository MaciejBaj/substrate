    ...
    V: Vm<T, Executable = E>,
{
    ...
    pub fn instantiate(
        ...
        let output = self.with_nested_context(dest.clone(), dest_trie_id, |nested| {
            ...
            debug::info!("Before transfer of endomwnent {:?} {:?} {:?} {:?}", endowment, gas_meter.gas_spent(), &dest, &caller);
            debug::info!("Before transfer of endomwnent balances {:?} {:?}", T::Currency::total_balance(&caller), T::Currency::total_balance(&dest));
            let r = transfer(
                gas_meter,
            ...
            debug::info!("After transfer of endomwnent {:?} ", r);
            ...
            debug::info!("subsistence_threshold  {:?} ", r);
...
fn transfer<'a, T: Trait, V: Vm<T>, L: Loader<T>>(
    ...
    if gas_meter.charge(ctx.config, token).is_out_of_gas() {
        ...
        debug::info!("Transfer FAILED OUT OF GAS");
    ...
    let existence_requirement = match (cause, origin) {
        ...
        (_, Contract) => {
            debug::info!("Transfer existence_requirement {:?} {:?} {:?}", transactor, T::Currency::total_balance(transactor), T::Currency::total_balance(transactor).saturating_sub(value));
